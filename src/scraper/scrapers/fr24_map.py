...
from src.scraper.models import FR24MapAircraftData, FR24MapResult, ScraperTask

logger = logging.getLogger("scraper.fr24_map")


class FR24MapScraper(BaseScraper[FR24MapResult]):
    ...
    def setup(self) -> None:
        ...
        if self.sync_to_database and self.database_url:
            try:
                ...
                logger.info("Database sync enabled for aircraft_realtime_positions")
            except Exception as e:
                logger.error(f"Failed to initialize database engine: {e}")
                self.sync_to_database = False

    def _ensure_table_exists(self) -> None:
        """Create the aircraft_realtime_positions table if it doesn't exist."""
        ...
        try:
            with self.db_engine.connect() as conn:
                ...
                conn.commit()
            logger.info("Ensured aircraft_realtime_positions table exists")
    ...
    def on_success(self, task: ScraperTask, result: FR24MapResult) -> None:
        ...
        Args:
            ...
            result: The scrape result with aircraft data.
        """
        super().on_success(task, result)

        # Save aircraft positions to database
        if self.sync_to_database and self.db_engine and result.aircraft:
            saved_count = self._save_positions_to_db(result)
            logger.info(
                f"[{task.task_key}] Saved {saved_count}/{len(result.aircraft)} "
                f"aircraft positions to database"
            )

    def _save_positions_to_db(self, result: FR24MapResult) -> int:
        """Save aircraft positions to the aircraft_realtime_positions table.

        Uses batch insert for performance, with upsert logic to avoid duplicates.

        Args:
            result: FR24MapResult containing aircraft to save.

        Returns:
            Number of positions saved/updated.
        """
        ...
        try:
            ...
            logger.error(f"[{result.task_key}] Failed to save positions: {e}")
            return 0
