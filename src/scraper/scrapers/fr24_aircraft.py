...
from src.scraper.models import FlightData, FR24AircraftResult, ScraperTask

logger = logging.getLogger("scraper.fr24_aircraft")


class FR24AircraftScraper(BaseScraper[FR24AircraftResult]):
    ...
    def setup(self) -> None:
        ...
        if self.sync_to_database:
            ...
            if self.db_engine:
                logger.info("Database sync enabled for FR24 aircraft scraper")
            else:
    ...
    def _save_flights_to_db(self, result: FR24AircraftResult) -> int:
        """Save flights to the flight_schedules table.

        Uses upsert logic based on (fr24_flight_id, DATE(scheduled_time), aircraft_registration).
        This ensures that the same flight on the same day updates existing records.

        Args:
            result: FR24AircraftResult containing flights to save.
        ...
        try:
            with self.db_engine.connect() as conn:
                for flight in result.flights:
                    ...
                    if not flight.scheduled_time:
                        logger.debug(
                            f"[{result.aircraft_registration}] Skipping flight "
                    ...
                    if not fr24_flight_id:
                        logger.debug(
                            f"[{result.aircraft_registration}] Skipping flight - "
                            f"no flight_id or flight_number"
                        )
                        continue

                    # For aircraft-centric scraping, we don't have a specific airport
                    # Set flight_type based on status or leave as None
                    ...
                    conn.execute(
                        ...
                        """),
                        {
                            ...
                            "airport_icao": None,  # Aircraft-centric, no specific airport
                            "airport_iata": None,
            ...
            if saved_count > 0:
                logger.info(
                    f"[{result.aircraft_registration}] Saved {saved_count} flights to flight_schedules"
                )
