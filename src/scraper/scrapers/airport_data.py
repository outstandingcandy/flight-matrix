...
- aircraft_static_info table updates
"""
...
from src.scraper.extractors import AirportDataExtractor
...
logger = logging.getLogger("scraper.airport_data")


class AirportDataScraper(BaseScraper[AirportDataResult]):
    ...
    Configuration options (in scraper config):
        ...
        database_url: Database URL for aircraft_static_info updates
    """
    ...
    def s3_client(self) -> Any:
        ...
        if self._s3_client is None and self.s3_upload:
    ...
    def db_engine(self) -> Any:
        ...
        if self._db_engine is None and self.database_url:
            self._db_engine = create_engine(self.database_url)
        return self._db_engine

    @property
    def task_queue(self) -> TaskQueue | None:
        """Get or create task queue."""
        if self._task_queue is None and self.database_url:
            self._task_queue = TaskQueue(self.database_url)
        return self._task_queue

    def _add_manufacturer_tasks(self, manufacturers: list[str]) -> int:
        """Add manufacturer scraping tasks to the queue.
        ...
        """
        if not self.task_queue or not manufacturers:
            return 0
    ...
    def _add_aircraft_detail_tasks(self, aircraft_list: list[AirportDataAircraftData]) -> int:
        """Add aircraft detail scraping tasks to the queue.

        Args:
            aircraft_list: List of aircraft data with registrations.

        Returns:
            Number of tasks added.
        """
        if not self.task_queue or not aircraft_list:
            return 0
    ...
    def _update_database(self, aircraft_list: list[AirportDataAircraftData]) -> int:
        """Update aircraft_static_info table with scraped data.

        Writes to both shared columns (manufacturer, model, etc.) and
        airport_data-specific columns with ad_ prefix.

        Args:
            aircraft_list: List of aircraft data to insert/update.

        Returns:
            Number of records updated.
        """
        if not self.database_url or not aircraft_list:
            return 0
        ...
        try:
            with self.db_engine.connect() as conn:
                for aircraft in aircraft_list:
                    try:
                        # Upsert into aircraft_static_info
                        # Write to both shared columns and ad_ prefixed columns
                        conn.execute(
                            text(
                                """
                            ...
                            ),
                            {
                                ...
                                "hex_code": getattr(aircraft, "mode_s_code", None),
                                "owner": getattr(aircraft, "owner", None),
                                "ad_status": getattr(aircraft, "status", None),
                                "ad_owner": getattr(aircraft, "owner", None),
                                "ad_engines": getattr(aircraft, "engines", None),
                                "ad_seats": getattr(aircraft, "seats", None),
                                "ad_location": getattr(aircraft, "location", None),
                                "ad_delivery_date": getattr(aircraft, "delivery_date", None),
                            },
                        )
                        updated += 1
