...
Supports both arrivals and departures with pagination and database persistence.
"""
...
from src.scraper.models import FlightData, FR24FlightsResult, ScraperTask

logger = logging.getLogger("scraper.fr24_airport")


class _FR24AirportScraper(BaseScraper[FR24FlightsResult]):
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        self.db_engine = None
        self.sync_to_database = self.config.get("sync_to_database", True)

    def setup(self) -> None:
        """Setup scraper resources."""
        super().setup()

        # Initialize database engine for syncing flights
        if self.sync_to_database and self.database_url:
            try:
                self.db_engine = create_engine(self.database_url, echo=False, pool_pre_ping=True)
                logger.info("Database sync enabled for flight_schedules")
            except Exception as e:
                logger.error(f"Failed to initialize database engine: {e}")
                self.sync_to_database = False
    ...
    def on_success(self, task: ScraperTask, result: FR24FlightsResult) -> None:
        ...
        Args:
            ...
            result: The scrape result with flight data.
        """
        super().on_success(task, result)

        # Sync flights to database
        if self.sync_to_database and self.db_engine and result.flights:
            self._save_flights_to_db(result)
            # Sync new aircraft registrations to aircraft_static_info
            # so JetPhotos scraper can automatically fetch their images
            self._sync_registrations_to_static_info(result)

    def _save_flights_to_db(self, result: FR24FlightsResult) -> int:
        """Save flights to the flight_schedules table.

        Uses upsert logic based on (fr24_flight_id, DATE(scheduled_time), flight_type).
        This ensures that the same flight on the same day updates existing records
        even if the exact scheduled_time changes (which is common on FR24).

        Args:
            result: FR24FlightsResult containing flights to save.

        Returns:
            Number of flights saved/updated.
        """
        ...
        try:
            with self.db_engine.connect() as conn:
                for flight in result.flights:
                    # Skip flights without any identification
                    if (
                    ...
                    # Skip flights without scheduled_time (required by DB constraint)
                    if not flight.scheduled_time:
                        logger.debug(
                            f"[{result.airport_code}] Skipping flight "
                            f"{flight.flight_number} - no scheduled_time"
                        )
                        continue

                    # Use flight_id or construct one
                    fr24_flight_id = flight.flight_id or flight.flight_number

                    # Skip flights without fr24_flight_id (required by DB constraint)
                    if not fr24_flight_id:
                        logger.debug(
                            f"[{result.airport_code}] Skipping flight with registration "
                            f"{flight.aircraft_registration} - no flight_id or flight_number"
                        )
                        continue

                    # Upsert: INSERT ON CONFLICT UPDATE
                    # Use (fr24_flight_id, DATE(scheduled_time), flight_type) for deduplication
                    # This ensures same flight on same day updates instead of creating duplicates
                    conn.execute(
                        text("""
                        ...
                        """),
                        {
                            "flight_type": flight.flight_type or self.flight_type,
            ...
            if saved_count > 0:
                logger.info(
                    f"[{result.airport_code}] Saved {saved_count} flights to flight_schedules"
                )
    ...
    def _sync_registrations_to_static_info(self, result: FR24FlightsResult) -> int:
        """Sync new aircraft registrations to aircraft_static_info table.

        Creates new records for aircraft registrations not yet in the database,
        enabling JetPhotos scraper to automatically fetch their images.

        Args:
            result: FR24FlightsResult containing flight data with registrations.

        Returns:
            Number of new aircraft_static_info records created.
        """
        if not self.db_engine or not result.flights:
            return 0

        # Extract valid registrations from flights
        ...
        try:
            ...
            if created_count > 0:
                logger.info(
                    ...
                    f"aircraft_static_info records from {len(registrations)} registrations"
                )
