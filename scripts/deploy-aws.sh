...
#   5. Hand off to ./deploy.sh deploy.
#
# Read docs/deployment.md for the full picture.
#
# Usage:
#   ./scripts/deploy-aws.sh            # interactive bootstrap, then deploy
#   ./scripts/deploy-aws.sh --check    # just verify prereqs + config, don't deploy
...
if (( CHECK_ONLY )); then
    ...
    echo "    ./scripts/deploy-aws.sh       # from scratch"
    echo "    ./deploy.sh deploy            # if you prefer the low-level script"
    exit 0
fi

cat <<EOF

${BOLD}About to run ${CYAN}./deploy.sh deploy${RESET}${BOLD}.${RESET}
...
This will create or update AWS resources and ${YELLOW}incur charges${RESET}.
EOF
